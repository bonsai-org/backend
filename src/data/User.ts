import { MongooseUtils } from '../utils/mongodb';
...
export class User {
  ...
  static async getByUsername(queryParams: {
    username: string;
  }): Promise<UserQuery> {
    try {
  ...
  static async getByEmailorUsername(queryParams: {
    email: string;
    username: string;
  }): Promise<UserQuery> {
    try {
      let user = await UserModel.findOne({
        $or: [{ username: queryParams.username }, { email: queryParams.email }],
      });
      return user;
    } catch (error) {
      throw new MongoError({
